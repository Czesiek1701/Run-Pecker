...
{
    //qDebug() << mmovables.getElements().size() << " " << mstables.getElements().size();
    for(auto* c: mmovables.getElements())
    {
        for(auto* w: mstables.getElements())
        {
            if( w->collidesWithItem( c ) )
            {
                (static_cast<Creature*>(c))->handleContact(*w);
            }
        }
    }
}
